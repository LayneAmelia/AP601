...
# Select relevant undergraduate race/ethnicity columns
undergrad <- data %>%
  select(Year = `...1`,
...
UG_resident_nonresident <- ggplot(undergrad, aes(x = as.integer(Year), y = Percent, fill = Category)) +
  geom_area(alpha = 0.8, size = 0, colour = "white") +  # Remove border lines between sections
  scale_fill_viridis_d(option = "plasma") +  # Use Viridis "inferno" palette
